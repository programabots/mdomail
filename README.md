...
4. Railway instalará las dependencias y ejecutará `npm run start` automáticamente. Los comandos *slash* se sincronizan al iniciar el bot siempre que `CLIENT_ID` esté configurado.
...
- ✅ Asegúrate de que `CLIENT_ID` esté configurado para que el bot sincronice los comandos al iniciar.
...
4. Despliega el servicio; Render instalará las dependencias y lanzará el bot con Node.js 18+. Los comandos *slash* se sincronizan automáticamente cuando el bot se inicia y encuentra `CLIENT_ID` configurado.
...
- ✅ Comprueba que `CLIENT_ID` esté definido para que el bot sincronice los comandos al iniciar.
