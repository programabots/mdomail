...
| `CLIENT_ID` | ID de la aplicación/bot. Necesario para registrar comandos. El bot sincroniza los comandos al iniciar si está definido. |
...
Si `CLIENT_ID` está definido, el bot sincroniza automáticamente los comandos en tu servidor cada vez que inicia (ideal para despliegues en Railway o Render).

Cuando quieras forzar la actualización manual desde tu máquina, ejecuta:

```bash
CLIENT_ID=tu_client_id npm run register
```

El script reutiliza la misma lógica que el bot en producción y sirve como comprobación rápida antes de desplegar.
...
- ✅ Asegúrate de que `CLIENT_ID` esté configurado para que el bot sincronice los comandos al iniciar o ejecútalos manualmente con `npm run register` si prefieres verificarlo de antemano.
...
- ✅ Comprueba que `CLIENT_ID` esté definido para que el bot sincronice los comandos al iniciar o ejecútalos manualmente (`npm run register`) desde un job si deseas confirmar los cambios.
